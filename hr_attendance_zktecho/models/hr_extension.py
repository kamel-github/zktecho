...
class hrDraftAttendance(models.Model):
    ...
    def automatically_sync_attendances(self):
        sync_days = 30
        date_to = datetime.today().date()
        date_from = date_to - timedelta(days=int(sync_days))
        ...
        for employee in employees:
            ...
            for rec in log_ids.sorted(key=lambda l: l.name):
                attendance_id = False
                if rec.attendance_status == 'sign_in':
                    vals = {
                        'employee_id': rec.employee_id.id,
                    ...
                    try:
                        previous_attendance_id = hr_attendance.search([('employee_id', '=', employee.id)],
                    ...
                    messages += "<p>%s <a href=# data-oe-model=%s data-oe-id=%d>%s</a></p>" % (
                    rec.employee_id.name, rec._name, rec.id, rec.name)
